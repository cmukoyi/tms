...
class ModuleDefinition(db.Model):
    ...
    # Relationships
    company_modules = db.relationship('CompanyModule', backref='module_definition', lazy='dynamic')
...
class CompanyModule(db.Model):
    """Which modules are enabled for each company - using your existing schema"""
    ...
    def get_company_modules_with_status(company_id):
        """Get all modules with their enabled status for a company"""
        # Get all modules
        all_modules = ModuleDefinition.get_all_modules()
